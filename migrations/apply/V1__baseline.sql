...
CREATE TABLE IF NOT EXISTS feed_source (id SERIAL PRIMARY KEY, source_id BIGINT NOT NULL, url TEXT NOT NULL, host_core TEXT NOT NULL, headline TEXT NOT NULL, host_name TEXT NULL, host_logo TEXT NULL, citation_count INT NOT NULL, word_count INT NOT NULL, "section" TEXT NULL, thumbnail TEXT NULL, seen_at TIMESTAMP NOT NULL, published_at TIMESTAMP NOT NULL, in_links JSON NOT NULL, scores JSON NOT NULL);
ALTER TABLE feed_source ADD CONSTRAINT feed_source_source_id_unique UNIQUE (source_id);
