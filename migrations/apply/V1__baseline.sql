...
CREATE TABLE IF NOT EXISTS feed (id SERIAL PRIMARY KEY, url TEXT NOT NULL, selector TEXT NOT NULL, "external" BOOLEAN DEFAULT FALSE NOT NULL, track_position BOOLEAN DEFAULT FALSE NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL);
CREATE TABLE IF NOT EXISTS source_score (id BIGSERIAL PRIMARY KEY, source_id BIGINT NOT NULL, origin_id BIGINT NULL, feed_id INT NULL, score INT NOT NULL, scored_at TIMESTAMP NOT NULL, CONSTRAINT fk_source_score_source_id__id FOREIGN KEY (source_id) REFERENCES "source"(id) ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT fk_source_score_origin_id__id FOREIGN KEY (origin_id) REFERENCES "source"(id) ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT fk_source_score_feed_id__id FOREIGN KEY (feed_id) REFERENCES feed(id) ON DELETE CASCADE ON UPDATE RESTRICT);
...
CREATE TABLE IF NOT EXISTS lead_job (id BIGSERIAL PRIMARY KEY, feed_id INT NULL, lead_id BIGINT NOT NULL, headline TEXT NULL, is_external BOOLEAN NOT NULL, fresh_at TIMESTAMP NULL, feed_position INT NULL, CONSTRAINT fk_lead_job_feed_id__id FOREIGN KEY (feed_id) REFERENCES feed(id) ON DELETE SET NULL ON UPDATE RESTRICT, CONSTRAINT fk_lead_job_lead_id__id FOREIGN KEY (lead_id) REFERENCES lead(id) ON DELETE CASCADE ON UPDATE RESTRICT);
...
CREATE SEQUENCE IF NOT EXISTS vector_model_id_seq START WITH 1 MINVALUE 1 MAXVALUE 9223372036854775807;
