...
CREATE TABLE IF NOT EXISTS host (id SERIAL PRIMARY KEY, nexus_id INT NULL, core TEXT NOT NULL, "name" TEXT NULL, logo TEXT NULL, robots_txt TEXT NULL, is_redirect BOOLEAN NULL, disallowed TEXT[] NOT NULL, domains TEXT[] NOT NULL, junk_params TEXT[] NOT NULL, CONSTRAINT fk_host_nexus_id__id FOREIGN KEY (nexus_id) REFERENCES nexus(id) ON DELETE SET NULL ON UPDATE RESTRICT);
