...
private val console = globalConsole.getHandle("PageScoreFinder")

class PageScoreFinder(
	...
	private suspend fun findScores() {
		val signals = pageScoreService.findScoreSignals(30.days)
		...
		val calculatedScores = signals.groupBy { it.targetId }.map { (targetId, list) ->
			...
			for (signal in topSignals) {
				...
				score += signalScore
				scores.add(PageScore(targetId, score, signal.linkedAt, signal.originId, null))
			}

			CalculatedScore(targetId, score, scores)
		}

		pageScoreService.addScores(calculatedScores)
